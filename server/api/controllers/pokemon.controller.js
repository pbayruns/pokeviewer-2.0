...
async function getAll(db) {
  const poke_SQL = "SELECT p.*, ps.*, pdn.pokedex_number AS 'order' FROM pokemon p JOIN pokemon_species ps ON (p.species_id = ps.id) JOIN pokemon_dex_numbers pdn ON (pdn.species_id = ps.id) WHERE pdn.pokedex_id = 1 ORDER BY 'order'";
